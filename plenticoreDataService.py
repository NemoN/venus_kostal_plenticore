...
class PlenticoreDataService:
    """Per-inverter data service with its own HTTP session and energy state."""

    def __init__(self, name=''):
        self.name = name
        self.http_session = requests.Session()
        self.http_session.headers.update({'Content-type': 'application/json', 'Accept': 'application/json'})
        self.lastTime = 0
        self.lastEnergy = 0
        self.calcEnergy = 0
        self._log_prefix = '[' + name + '] ' if name else ''
    ...
    def reset_energy_state(self):
        ...
        self.lastEnergy = 0
        self.calcEnergy = 0
        logger.info(self._log_prefix + 'Energy calculation state reset')

    def get_data(self, baseUrl, sessionId):
        self.http_session.headers['authorization'] = "Session " + sessionId
        url = baseUrl + "/processdata/devices:local:ac"

        response = self.http_session.get(url=url, timeout=REQUEST_TIMEOUT)
        ...
        try:
            ...
            response = self.http_session.post(url=url, json=batch_payload, timeout=REQUEST_TIMEOUT)
            response.raise_for_status()
            ...
            logger.warning(self._log_prefix + 'Failed to fetch DC tracker data: ' + str(err))
            for i in range(1, 4):
                prefix = 'PV{}_'.format(i)
                data[prefix + 'U'] = None
                data[prefix + 'I'] = None
        ...
        energy = next((x['value'] for x in yield_data if x['id'] == 'Statistic:Yield:Total'), None)
        if energy is None:
            logger.warning(self._log_prefix + 'Statistic:Yield:Total missing from inverter response, using last known energy value')
            energy = self.lastEnergy
        data['YIELD_DAY'] = round(next((x['value'] for x in yield_data if x['id'] == 'Statistic:Yield:Day'), 0.0) / 1000.0, 3)
        currentTime = time.time()

        if energy != self.lastEnergy:
            logger.info(self._log_prefix + "Calculated Energy: {} Energy: {}".format(self.calcEnergy, energy))
            self.lastEnergy = energy
            self.calcEnergy = energy
            self.lastTime = currentTime
        else:
            deltaTime = currentTime - self.lastTime
            if deltaTime > 300:
                logger.warning(self._log_prefix + 'Large time gap ({:.0f}s), capping delta to 300s'.format(deltaTime))
                deltaTime = 300
            delta = data['PT'] * deltaTime / 3600
            self.calcEnergy += delta
            logger.debug(self._log_prefix + "Calculated Energy: {} Energy: {} Last Energy {}.".format(self.calcEnergy, energy, self.lastEnergy))

        data['EFAT'] = round(self.calcEnergy / 1000.0, 3)
